...
export const useTradingComposeTransaction = <T extends TradingSellExchangeFormProps>({
    ...
    const outputAmount = values?.outputs?.[0]?.amount;
    const setMaxOutputId = values?.setMaxOutputId;
    const prevFromAddressInputs = useRef<string | undefined>(undefined);

    useEffect(() => {
        if (type !== 'exchange' || account.networkType !== 'bitcoin') return;
        if (!outputAmount && typeof setMaxOutputId !== 'number') return;
