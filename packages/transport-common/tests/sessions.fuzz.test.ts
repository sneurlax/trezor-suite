...
const runChurn = async (
    ...
    const state: RunState = { owners: [], maxOwners: 0, observedCurrent: null };
    ...
    // counting as a standing owner.
    background.on('releaseRequest', descriptor => {
        const notified = state.owners.findIndex(o => o.session === descriptor.session);
    ...
    try {
        ...
        for (const pick of script) {
            advance(actors[pick % clientCount], state, options);
...
describe('sessions fuzz (concurrency + stealing)', () => {
    ...
    it('VERIFY: a release racing a steal must not destroy the new owner session', async () => {
        ...
        try {
            ...
            // c1 owns "2"; the device must still reflect that
