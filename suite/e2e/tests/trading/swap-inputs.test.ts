...
test.describe('Trading - Swap inputs', { tag: ['@webOnly', '@noDevice'] }, () => {
    ...
    test('Swap form inputs validation', async ({ page, indexedDb, walletPage, tradingPage }) => {
        ...
        for (const [index, asset] of buyAssets.entries()) {
            ...
            await test.step(`[${asset.label}] Select receive account`, async () => {
                ...
                await tradingPage.receiveAccount.verifySelectedReceiveAccount(selectedAccountName);
            });
