...
const GWEI_TO_WEI = 1_000_000_000;

const gweiToWei = (value: string) => new BigNumber(value).times(GWEI_TO_WEI).toFixed(0);
...
export const getYieldClaimEip1559Fee = (
    feePreview: PrecomposedTransactionFinal,
): ClaimFee | null => {
    if (!feePreview.feeLimit || !feePreview.maxFeePerGas || !feePreview.maxPriorityFeePerGas) {
        return null;
    }
