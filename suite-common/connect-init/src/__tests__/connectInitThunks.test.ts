...
import { connectInitThunk, defaultTrezorUIEventHandlerThunk } from '../connectInitThunks';
...
describe('TrezorConnect Actions', () => {
    ...
    it('callId-bearing UI events are swallowed by the global listener', async () => {
        ...
        // Plain UI event (no callId): flows through defaultTrezorUIEventHandlerThunk,
        // producing pending + the dispatched action + fulfilled.
        emitTestEvent(UI_EVENT, {
            type: UI_REQUEST.REQUEST_BUTTON,
            payload: { code: 'ButtonRequest_ProtectCall' },
        });
        // callId-bearing UI event: must be dropped on entry, never reach the thunk.
        emitTestEvent(UI_EVENT, {
            type: UI_REQUEST.REQUEST_BUTTON,
            payload: { code: 'ButtonRequest_ProtectCall' },
            callId: 'scoped-call-id',
        });
        // Drain microtasks so the async thunk dispatched by the plain event
        // gets to emit its /fulfilled action.
        await new Promise(resolve => setImmediate(resolve));

        const newActions = store.getActions().slice(actionsBefore);

        // The plain event produced thunk pending / its raw type / thunk fulfilled.
        // The scoped event must not have added any further actions of that kind.
    ...
    it('connectInitHooks.uiEvent is called per action.type forwarded from the global listener', async () => {
        ...
        // Unrelated UI event must NOT fire either hook.
        emitTestEvent(UI_EVENT, {
            type: UI_REQUEST.REQUEST_BUTTON,
            payload: { code: 'ButtonRequest_ProtectCall' },
        });
